...
def _time_adjustment_and_sta_file(nt_code, st_code, bg, ed):
    ...
    #make a value for the end date if it is ongoing
    if st_data[0][0].end_date is None:
        st_data[0][0].end_date = obspy.UTCDateTime.now()
            ...
            for ch in ch_list:
                ...
                if len(wv_ch) >= 1:
                    ...
                    try:
                        ...
                        file_name = f"{wv_ch[0].id}__{wv_ch[0].stats.starttime.strftime('%Y%m%dT%H%M%SZ')}__{wv_ch[0].stats.endtime.strftime('%Y%m%dT%H%M%SZ')}.mseed"
                        wv_ch.write(f"{folder}/{file_name}", format='MSEED')
                        print(f"                \u2713 \u2713{file_name} saved \u2713 \u2713")
                    except:
                        print(f"                \u00D7 \u00D7 There is a problem with matching the channel respose - file NOT saved! \u00D7 \u00D7")
                else:
